    ...
    for y in range(1,days_in_month+1):
        ...
        if(row[0]== str(month) and row[1] == str(y) ):
            ...
            if( hijri_date.day_name()) == "Friday":
                date_values=[row[1],row[2],row[8],row[3],row[4],'1:00',row[5],row[10],row[6],row[7],row[11],hijri_date.month_name(),hijri_date_tuple[2],hijri_date_tuple[0],current_month_name,year,hijri_date.day_name()]
            else:    
